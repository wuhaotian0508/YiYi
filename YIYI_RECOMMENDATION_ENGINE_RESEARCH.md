...
Live Realtime uses the repository’s installed `@openai/agents` API (`RealtimeAgent`, `RealtimeSession`, WebRTC, `semantic_vad` with low eagerness). The application adds cancellation/generation guards around token acquisition and session replacement, while leaving transport, interruption, and audio mechanics to the SDK.
