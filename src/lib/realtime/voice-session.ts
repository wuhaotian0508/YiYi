...
import { DailyIntentSchema, IntentDeltaSchema, type IntentDelta } from "@/domain/schemas";
...
function toolsFor(handlers: VoiceToolHandlers) {
  return [
    tool({ name: "request_outfit_recommendation", description: "Select one decisive outfit from an internal pool of legal candidates.", parameters: DailyIntentSchema, strict: true, timeoutMs: 25_000, execute: (input) => handlers.requestRecommendation(input) }),
    tool({ name: "revise_current_outfit", description: "Return a complete structured delta for targeted, global, random, or undo operations. Encode warmth, comfort, formality, colorfulness, preservation, required items, and exclusions explicitly; rawUtterance alone never changes the outfit.", parameters: IntentDeltaSchema, strict: true, timeoutMs: 20_000, execute: (input) => handlers.revise(input) }),
...
export class OpenAIRealtimeVoiceAdapter implements VoiceSessionAdapter {
  ...
  async connect() {
    ...
    try {
      const response = await fetch("/api/realtime/token", { method: "POST", headers: { "Content-Type": "application/json" }, cache: "no-store", signal: connectAbortController.signal });
      ...
      session.on("error", () => { if (isActive()) this.emitState("error"); });
      ...
      session.transport.on("connection_change", (status) => {
        ...
        if (status === "disconnected") this.emitState("error");
      });
  ...
  async disconnect() { this.connectGeneration += 1; this.connectAbortController?.abort(); this.connectAbortController = null; const session = this.session; this.session = null; session?.close(); this.emitState("idle"); }
