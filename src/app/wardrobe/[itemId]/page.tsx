...
export default function ItemDetailPage() {
  ...
  return <main className="phone-page"><div className="page-column"><header className="topbar"><Link href="/wardrobe" className="icon-button" aria-label="Back"><ChevronLeft /></Link><span /><span /></header><motion.div className="item-detail-visual" initial={reduceMotion ? { opacity: 0 } : { opacity: 0, scale: .94, y: 8 }} animate={{ opacity: 1, scale: 1, y: 0 }} transition={reduceMotion ? { duration: .12 } : calmSpring}><Garment item={item} /></motion.div><motion.header className="item-detail-heading" initial={reduceMotion ? { opacity: 0 } : { opacity: 0, y: 5 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: reduceMotion ? .1 : .24, delay: reduceMotion ? 0 : .1 }}><h1>{item.subtype}</h1><p>{item.category.replaceAll("_", " ")}</p></motion.header><div className="item-detail-attributes">{rows.map(([label, value], index) => <motion.div className="attribute-row" key={label} initial={reduceMotion ? { opacity: 0 } : { opacity: 0, y: 4 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: reduceMotion ? .1 : .22, delay: reduceMotion ? 0 : .14 + index * .035 }}><span>{label}</span><span className={label === "Availability" ? "availability-value" : "attribute-value"}>{label === "Color" && <i style={{ width: 10, height: 10, borderRadius: "50%", background: colorHex[item.primaryColor] }} />}{value}{label === "Availability" && <Check size={14} />}</span></motion.div>)}</div><motion.div className="item-detail-actions" initial={reduceMotion ? { opacity: 0 } : { opacity: 0, y: 5 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: reduceMotion ? .1 : .24, delay: reduceMotion ? 0 : .36 }}><SecondaryButton onClick={() => void toggleAvailability()}>{item.availability === "available" ? "Mark as unavailable" : "Mark as available"}</SecondaryButton><AnimatePresence mode="wait" initial={false}>{confirmDelete ? <motion.div className="delete-confirm" key="confirm" initial={{ opacity: 0, y: 6 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: 4 }}><p>Delete this item and its local images?</p><div><SecondaryButton onClick={() => setConfirmDelete(false)}>Keep item</SecondaryButton><PrimaryButton onClick={() => void deleteItem()}>Delete</PrimaryButton></div></motion.div> : <motion.button key="delete" className="secondary-button danger-button" onClick={() => setConfirmDelete(true)} initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>Delete item</motion.button>}</AnimatePresence></motion.div></div></main>;
}
