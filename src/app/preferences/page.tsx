...
export default function PreferencesPage() {
  ...
  return <main className="phone-page"><div className="page-column"><header className="topbar"><Link href="/today" className="icon-button" aria-label="Back"><ChevronLeft /></Link><YiYiMark size={37} /><span /></header><h1 className="memory-title">What YiYi remembers</h1>{sections.map((section) => <section className="memory-section" key={section.id}><div className="memory-section-heading"><h2>{section.title}</h2><button className="chip" onClick={() => { setEditing(section.id); setDraft(""); }}>Edit</button></div>{section.chips ? <div className="chip-row memory-chips">{section.values.map((value) => <span className="chip" key={value}>{value}</span>)}</div> : section.values.map((value) => <p className="secondary-copy" key={value}>{value}</p>)}</section>)}<section className="memory-explanation"><h2>YiYi only saves preferences</h2><p className="secondary-copy">you state clearly or repeat over time.</p></section>{confirmReset ? <div className="reset-confirm"><p>Reset all saved preferences to the demo defaults?</p><div><SecondaryButton onClick={() => setConfirmReset(false)}>Cancel</SecondaryButton><PrimaryButton onClick={() => void resetPreferences()}>Reset</PrimaryButton></div></div> : <button className="secondary-button danger-button memory-reset" onClick={() => setConfirmReset(true)}>Reset preferences</button>}<BottomSheet open={Boolean(editing)} onClose={() => setEditing(null)} label="Edit saved preferences" className="preference-sheet">{editing && <PreferenceSheet section={editing} values={sections.find((section) => section.id === editing)?.values ?? []} draft={draft} onDraft={setDraft} onAdd={() => void addMemory()} onRemove={(value) => void removeMemory(editing, value)} onClose={() => setEditing(null)} />}</BottomSheet></div></main>;
}
