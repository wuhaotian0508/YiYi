...
The browser requests an ephemeral secret from `/api/realtime/token`, then connects a persistent `RealtimeSession` with that secret. The official SDK manages WebRTC. YiYi uses `semantic_vad` with `eagerness: high` for prompt first-turn handoff, automatic response, and interruption. A new Today session starts with the named `request_outfit_recommendation` tool choice and restores `auto` as soon as that tool begins, so later revision and confirmation turns remain conversational without allowing the first turn to end as speech-only. Pending token fetches are abortable; obsolete listener, role-separated transcript, and tool events are rejected by session generation. A committed Dexie mutation remains locked through local publish, while a still-preparing mutation can be cancelled. Raw `RTCPeerConnection`, SDP, media-track plumbing, audio playback buffers, and long-lived server keys are forbidden in client code.
