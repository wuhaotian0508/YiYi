...
| `decision:delete` | Every character disappears from the end; slow first, then faster | Cursor hold complete | Entire indecisive paragraph is removed in reverse order | 15.35–18.75 s | 1.70 s empty field | Reframe starts | Yes; Skip kills timeline and clears caret | Indecisive paragraph crossfades out as one state, then holds empty for 1.65 s | GSAP TextPlugin |
| `reframe:start` | User expresses the day and desired feeling in the same location | Empty hold complete | `I have class, dinner with friends,\nand a lot of walking.` | 20.45–24.60 s | 0.45 s | Second sentence | Yes | First reframe sentence fades in complete after a 1.65 s empty hold | GSAP TextPlugin |
...
| `complete` | Updated reason and “Everything else stayed.” | Replacement settled | Final causal explanation | After replacement settles | User-controlled hold | Continue advances to permission | Skip remains available until exit | Final state remains until Continue | GSAP outer scene |
